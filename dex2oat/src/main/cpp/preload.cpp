...
constexpr char kDex2OatPath[] = "/apex/com.android.art/bin/dex2oat";
...
__attribute__((constructor)) static void initialize() {
    ...
    if (maps == nullptr) {
        LOGE("lsplt_scan_maps failed");
        return;
    }

    for (size_t i = 0; i < maps->length; ++i) {
        auto& info = maps->maps[i];
        if (info.path != nullptr && strncmp(info.path, kDex2OatPath, sizeof(kDex2OatPath) - 1) == 0) {
            dev = info.dev;
            inode = info.inode;
            break;
        }
    ...
    if (dev == 0 && inode == 0) {
        LOGE("failed to locate dex2oat mapping, skipping hooks");
        return;
    }
